...
export class Init1715098463423 implements MigrationInterface {
    name = 'Init1715098463423'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`modules\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`status\` varchar(30) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`label\` varchar(50) NOT NULL COMMENT 'Etiqueta del módulo para el Sidebar del proyecto', \`url\` varchar(50) NOT NULL COMMENT 'Ruta para acceder al módulo', \`name\` varchar(50) NOT NULL COMMENT 'Nombre del módulo', \`icon\` varchar(50) NULL COMMENT 'Icono del modulo', \`description\` varchar(50) NOT NULL COMMENT 'Descripcion del modulo', \`order\` int NOT NULL COMMENT 'Ruta para acceder al módulo', \`id_module\` bigint NULL COMMENT 'Clave foránea que índica que pertenece a otro módulo', UNIQUE INDEX \`IDX_b32d2596a6c578bf737e151747\` (\`url\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
